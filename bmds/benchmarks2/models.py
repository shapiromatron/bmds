...
from sqlmodel import Field, SQLModel, Column, PickleType, Field, Relationship
...
class TblSession(SQLModel, table=True):
    ...
    def get_df(cls,objs:"TblSession",follow:bool=True):
        records = [obj.dict(include={"id","bmds_version","os","dataset_name","dtype"}) for obj in objs]
...
class TblModel(SQLModel, table=True):
    ...
    def get_df(cls,objs:"TblModel",follow:bool=True):
        records = [obj.dict(include={"id","session_id","model_name","settings_name"}) for obj in objs]
...
class TblModelResultScalar(SQLModel, table=True):
    ...
    @classmethod
    def from_model(cls,model:TblModel,attrs:list[constants.ScalarAttribute])->list["TblModelResultScalar"]:
